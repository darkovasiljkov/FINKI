...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> firstCard = firstFriendCards.getFirst();
        SLLNode<Card> secondCard = secondFriendCards.getFirst();
        SLLNode<Card> max=null;
        int max2=0;
        while (firstCard != null)
        {
            if (firstCard.element.damage() > max2)
            {
                max2=firstCard.element.damage();
                max=firstCard;
            }
            firstCard=firstCard.succ;
        }

        Card deletedCard = firstFriendCards.delete(max);

        for (int i = 0; i < 2; i++)
            secondCard= secondCard.succ;

        secondFriendCards.insertAfter(deletedCard, secondCard);
    }
